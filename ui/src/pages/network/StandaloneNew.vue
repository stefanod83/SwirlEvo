<template>
  ...
  <n-space class="page-body" vertical :size="12">
    <n-alert v-if="!selectedHostId" type="warning" :show-icon="true">
      {{ t('texts.select_host_body') || 'Choose a host from the top bar before creating a network.' }}
    </n-alert>
    <n-form v-else :model="model" :rules="rules" ref="form" label-placement="top" label-width="90">
      <n-grid cols="1 640:2" :x-gap="24">
        <n-form-item-gi :label="t('fields.name')" span="2" path="name" label-placement="left">
          <n-input :placeholder="t('fields.name')" v-model:value="model.name" style="max-width: 400px" />
        </n-form-item-gi>
        ...
        <n-form-item-gi :label="t('fields.internal')" path="internal" label-placement="left" label-align="right">
        ...
        <n-form-item-gi span="2" label="IP" path="ipam.config">
          <n-dynamic-input v-model:value="model.ipam.config" #="{ index, value }" :on-create="newConfig">
            <n-input-group>
        ...
        <n-form-item-gi span="2" :label="t('fields.options')" path="options">
        ...
        <n-form-item-gi span="2" :label="t('fields.tags')" path="labels">
...
import {
  NButton, NSpace, NInput, NIcon, NForm, NGrid, NGi, NFormItemGi, NSwitch,
  NDynamicInput, NRadioGroup, NRadio, NInputGroup, NInputGroupLabel, NAlert,
} from "naive-ui";
import {
...
const { submit, submiting } = useForm(form,
  () => networkApi.save(model.value, selectedHostId.value || ''),
  () => {
